...
interface IRoute {
  price: { amountReturned: string; estimatedGas: string; tokenApprovalAddress: string; logo: string; feeAmount?: number } | null;
  name: string;
  airdrop: boolean;
  fromAmount: string;
  txData: string;
...
export async function getAdapterRoutes({ adapter, chain, from, to, amount, extra = {} }: IGetAdapterRouteProps): Promise<IRoute> {
  if (!chain || !from || !to || !amount || amount === '0') {
    return { price: null, name: adapter.name, airdrop: !adapter.token, fromAmount: amount, txData: '', l1Gas: 0, selectedChain: chain };
  }
    ...
    queries: adapters
      ...
      .map<UseQueryOptions<IRoute>>((adapter) => {
        return {
          ...
          onSuccess: (data) => {
            console.log(`Route Data from ${adapter.name}:`, data); // Debugging log
